...
class Kitti3DBox(_BaseDataset):
        ...
        for t in range(num_timesteps):
            ...
            if is_gt:
                if time_key in ignore_data.keys():
                    ...
                    raw_data['gt_crowd_ignore_regions'][t] = np.atleast_2d(time_ignore[:, 6:17])
                else:
    ...
    def _calculate_3d_box_corners(self, bboxes3d_input):
        bboxes3d_output = []
        for bbox3d_input in bboxes3d_input:
            R = np.array([[np.cos(bbox3d_input[-1]),  0,  np.sin(bbox3d_input[-1])],
                         [0,  1,  0],
                         [-np.sin(bbox3d_input[-1]), 0,  np.cos(bbox3d_input[-1])]])
            ...
            corners_3d[0, :] = corners_3d[0, :] + bbox3d_input[3]
            corners_3d[1, :] = corners_3d[1, :] + bbox3d_input[4]
            corners_3d[2, :] = corners_3d[2, :] + bbox3d_input[5]

            bboxes3d_output.append(np.transpose(corners_3d))

        return np.array(bboxes3d_output)
