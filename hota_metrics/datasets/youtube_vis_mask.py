...
class YouTubeVisMask(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/youtube_vis/'),  # Trackers location
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for t in range(num_timesteps):
            ...
            raw_data['classes'][t] = np.atleast_1d([track['category_id'] for track in tracks
                                                    if track['segmentations'][t]]).astype(int)
            if is_gt:
                raw_data['is_crowd'][t] = np.atleast_1d([track['iscrowd'] for track in tracks
                                                         if track['segmentations'][t]]).astype(int)
            else:
